...
function generatePassword() {
    ...
    do {
//the pop up messages
     has_lower = confirm("Do you want to have any lowercase?");
     has_numeric = confirm("Do you want any numerals?");
     has_symbols = confirm("Do you wany symbols?");
    has_upper = confirm("do you want uppercase?");
...
// each of these create 
        var has_numeric_check = !has_numeric;
        for (var i = 0; i < gen_password.length; i++) 
        {
            if (letters_numeric.indexOf(gen_password.charAt(i)) >= 0) 
            {
                has_numeric_check = true;
                console.log("found numeric!" + gen_password.charAt(i));
                break;
            }
        }

        var has_lower_check = !has_lower;
        for (var i = 0; i < gen_password.length; i++) 
        {
            if (letters_lower.indexOf(gen_password.charAt(i)) >= 0) 
            {
                has_lower_check = true;
                console.log("found lower!" + gen_password.charAt(i));
                break;
            }
        }

        var has_upper_check = !has_upper;
        for (var i = 0; i < gen_password.length; i++) 
        {
            if (letters_upper.indexOf(gen_password.charAt(i)) >= 0) 
            {
                has_upper_check = true;
                console.log("found upper!" + gen_password.charAt(i));
                break;
            }
        }

        var has_symbols_check = !has_symbols;
        for (var i = 0; i < gen_password.length; i++) 
        {
            if (letters_symbols.indexOf(gen_password.charAt(i)) >= 0) 
            {
                has_symbols_check = true;
                console.log("found symbols!" + gen_password.charAt(i));
                break;
            }
        }
